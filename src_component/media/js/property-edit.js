    ...
    (function() {
        ...
        function updateSubRegions(callback) {
            ...
            if (mainRegionId && mainRegionId !== '') {
                // Joomla admin forms require a security token for AJAX requests.
                const token = Joomla.getToken();
                if (!token) {
                    console.error('Joomla security token not found!');
                    return;
                }
                const url = `index.php?option=com_bookingmanager&task=subregions.getSubRegions&format=json&main_region_id=${mainRegionId}&${token}=1`;
